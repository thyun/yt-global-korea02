...
### 원본 영상 활용 원칙 (03~04단계 핵심)

- **원본 영상 시청 욕구 자극**: 나레이션은 원본 영상의 주요 포인트(핵심 발언·장면·반응)를 중심으로 작성한다. 시청자가 "직접 보고 싶다"는 욕구를 느끼도록, 결정적 장면은 나레이션으로 충분히 설명하되 원본 화자의 실제 표정·반응은 직접 보여주는 방식으로 구성한다.
- **원본 클립 삽입 횟수**: 나레이션 중간에 **2~4회** 원본 영상 클립을 삽입한다. 매 씬마다 넣지 않고, 시청자의 호기심이 최고조에 달하는 순간에 집중 배치한다.
- **삽입 타이밍 원칙**:
  1. 나레이션으로 궁금증을 충분히 쌓은 뒤 → 원본 클립으로 해소
  2. 핵심 반응·발언 장면에만 배치 (일상 B롤 구간 지양)
  3. 클립 1회당 30초~2분 이내로 제한
- `script.md` 작성 시 원본 클립 삽입 위치에 `[소스 클립 MM:SS~MM:SS]` 형식으로 명시한다.
